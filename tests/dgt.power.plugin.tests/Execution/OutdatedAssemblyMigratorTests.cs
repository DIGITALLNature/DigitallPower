...
public class OutdatedAssemblyMigratorTests
{
    ...
    [Test]
    public async Task MigrateAsync_MatchingType_MigratesCustomApiLinkButKeepsOldAssembly()
    {
        var (service, migrator) = CreateMigrator();
        ...
        // Old assembly/type are left untouched since PurgeOutdated was not set.
    ...
    [Test]
    public async Task MigrateAsync_DryRun_DoesNotMigrateAnything()
    {
        var (service, migrator) = CreateMigrator();
        var oldAssemblyId = CreateAssembly(service, "MyPlugins", "1.0.0.0");
        var oldTypeId = CreateType(service, oldAssemblyId, "MyPlugin");
    ...
    [Test]
    public async Task MigrateAsync_PurgeOutdated_MigratesStepsAndDeletesOldAssembly()
    {
        var (service, migrator) = CreateMigrator();
        var oldAssemblyId = CreateAssembly(service, "MyPlugins", "1.0.0.0");
        var oldTypeId = CreateType(service, oldAssemblyId, "MyPlugin");
        ...
        await migrator.MigrateAsync(
            "MyPlugins", newAssemblyId, replacementTypes, new PluginPushOptions(null, DryRun: false, PurgeOutdated: true));
    ...
    [Test]
    public async Task MigrateAsync_PurgeOutdated_TypeWithoutReplacement_DeletesOrphanedStepsAndType()
    {
        var (service, migrator) = CreateMigrator();
        var oldAssemblyId = CreateAssembly(service, "MyPlugins", "1.0.0.0");
        ...
        await migrator.MigrateAsync("MyPlugins", newAssemblyId, [], new PluginPushOptions(null, DryRun: false, PurgeOutdated: true));
