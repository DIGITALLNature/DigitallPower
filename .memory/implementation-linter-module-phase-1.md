...
Note: this module was originally named `dgt.power.linter` and has since been renamed to
`dgt.power.solution` (CLI branch `lint run --solutions <sol1,sol2>` → `solution lint <Solution>`),
see `decision-resource-oriented-cli-restructuring.md`. `LintVerb`/`LintRunCommand` below are now
`SolutionLintSettings`/`SolutionLintCommand`; everything else described here (`ILintRule`,
`LintContext`, `LintRuleCatalog`, rule behavior) is unchanged.
...
`TryGetProperty` parsing per rule. `schemas/solution/lint/schema.json` gives each rule id its own closed
